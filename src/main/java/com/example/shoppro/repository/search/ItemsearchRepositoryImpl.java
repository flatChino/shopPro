...
public class ItemsearchRepositoryImpl extends QuerydslRepositorySupport implements ItemsearchRepository {
    ...
    public Page<Item> getAdminItemPage(PageRequestDTO pageRequestDTO, Pageable pageable, String email) {
        //만들 기능들 : 1.판매중 품절 여부 2.날 짜에 따른 검색 3.만든이(중요함) 4.아이템이름
        //select * from where reg= ww;
        ...
        if(pageRequestDTO.getSearchDateType()==null||pageRequestDTO.getSearchDateType().equals("all")||
                pageRequestDTO.getSearchDateType().equals("")){


        }else if(searchDateType.equals("id")){
...
//                if(str.equals("t"))
        ...
        query.where(item.CreateBy.matches(email)); // 현재판매자
